...
public class MeetingController {
	...
	protected ModelAndView select(MeetingVO vo, String keyword, String action) {
		...
		if (action != null && action.equals("insert")) {
			...
			vo.setId(Integer.parseInt(action)); // action이 숫자=id 니까
			boolean result = dao.update(vo);
			if (result) {
				mav.addObject("msg", vo.getId() + "번 글이 수정되었습니다.");
			} else {
				mav.addObject("msg", vo.getId() + "번 글이 수정되지 않았습니다.");
			}
		}
		
		else if (vo.getName() != null && vo.getMeetingDate() != null && vo.getTitle() != null) {
			boolean result = dao.insert(vo);
			if (result) {
				mav.addObject("msg", "성공적으로 저장되었어요.");
			} else {
